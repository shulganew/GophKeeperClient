...
func (lm *GtextMenu) GetInit() tea.Cmd {
	return nil
}
...
func (lm *GtextMenu) GetUpdate(m *tui.Model, msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		switch msg.String() {
		case "ctrl+c", "esc":
			m.ChangeState(tui.GtextMenu, tui.MainMenu)
			return m, nil
		case "down":
			lm.Choice++
		...
		case "enter":
			...
			// List logins/pw.
			case 0:
				m.ChangeState(tui.GtextMenu, tui.GtextAdd)
			// Add NEW.
			case 1:
				m.ChangeState(tui.GtextMenu, tui.GtextList)
			}
			return m, nil
		}
	}
