...
func CardAdd(conf config.Config, jwt, def, ccn, cvv, exp, hld string) (card *oapi.NewCard, status int, err error) {
	...
	card = &oapi.NewCard{Definition: def, Ccn: ccn, Cvv: cvv, Exp: exp, Hld: hld}
	// Add saved jwt token for auth.
	resp, err := c.AddCard(context.TODO(), *card, func(ctx context.Context, req *http.Request) error {
